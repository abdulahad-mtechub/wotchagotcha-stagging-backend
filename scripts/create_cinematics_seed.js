...
// This script inserts 10 unique records per cinematics subcategory directly into the DB
// Usage: `node scripts/create_cinematics_seed.js`
...
async function main() {
  ...
  try {
    ...
    const cats = await client.query('SELECT id,name FROM cinematics_category ORDER BY id');
    if (cats.rowCount === 0) {
      console.error('No rows in cinematics_category. Aborting.');
      process.exit(1);
    }
    ...
    for (const cat of cats.rows) {
      const subsRes = await client.query('SELECT id,name FROM cinematics_sub_category WHERE category_id=$1 ORDER BY id', [cat.id]);
      let subs = subsRes.rows;
      if (subs.length === 0) {
        // fallback to any subcategory
        const anySub = await client.query('SELECT id,name FROM cinematics_sub_category LIMIT 1');
        if (anySub.rowCount === 0) {
          console.error('No cinematics_sub_category rows found at all. Aborting.');
      ...
      console.log(`Inserting ${POSTS_PER_SUBCATEGORY} items for cinematics_category id=${cat.id} (${cat.name})`);

      for (const sub of subs) {
        for (let i = 1; i <= POSTS_PER_SUBCATEGORY; i++) {
          const sample = SAMPLES[(i - 1) % SAMPLES.length];
          const name = uniqueText(`${cat.name} / ${sub.name} Showcase`, i);
          const description = uniqueText(`Auto-inserted cinematic for ${cat.name} - ${sub.name}`, i);
          const insertQ = `INSERT INTO cinematics_videos (user_id, name, description, category_id, sub_category_id, video, thumbnail) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING *`;
