...
// Inserts 10 unique records per tv_progmax subcategory into `tv_progmax_videos`
// Usage: `node scripts/create_tvprogmax_seed.js`
...
async function main() {
  ...
  try {
    ...
    const cats = await client.query('SELECT id,name FROM tv_progmax_category ORDER BY id');
    if (cats.rowCount === 0) {
      console.error('No rows in tv_progmax_category. Aborting.');
      process.exit(1);
    }
    ...
    for (const cat of cats.rows) {
      const subsRes = await client.query('SELECT id,name FROM tv_progmax_sub_category WHERE category_id=$1 ORDER BY id', [cat.id]);
      let subs = subsRes.rows;
      if (subs.length === 0) {
        const anySub = await client.query('SELECT id,name FROM tv_progmax_sub_category LIMIT 1');
        if (anySub.rowCount === 0) {
          console.error('No tv_progmax_sub_category rows found at all. Aborting.');
      ...
      console.log(`Inserting ${POSTS_PER_SUBCATEGORY} items for tv_progmax_category id=${cat.id} (${cat.name})`);

      for (const sub of subs) {
        for (let i = 1; i <= POSTS_PER_SUBCATEGORY; i++) {
          const sample = SAMPLES[(i - 1) % SAMPLES.length];
          const name = uniqueText(`${cat.name} / ${sub.name} Program`, i);
          const description = uniqueText(`Auto-inserted tv program for ${cat.name} - ${sub.name}`, i);
          const insertQ = `INSERT INTO tv_progmax_videos (user_id, name, description, category_id, sub_category_id, video, thumbnail) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING *`;
