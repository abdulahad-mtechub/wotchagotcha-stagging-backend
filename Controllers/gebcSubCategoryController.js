...
export const create = async (req, res) => {
  try {
    ...
    const createQuery =
      "INSERT INTO public.GEBC_sub_category (category_id, name) VALUES ($1, $2) RETURNING *";
...
export const deleteCategory = async (req, res) => {
  ...
  try {
    ...
    const delQuery = "DELETE FROM public.GEBC_sub_category WHERE id=$1";
...
export const getSpecificCategory = async (req, res) => {
  try {
    ...
    const result = await getSingleRow("public.GEBC_sub_category", condition);
    if (result.length === 0) {
      return res
        .status(404)
        .json({ statusCode: 404, message: "Sub-category not found" });
...
export const getAllSubCategories = async (req, res) => {
  try {
    ...
    let subCategoryQuery = `
          SELECT sc.*
          FROM public.GEBC_sub_category sc
    ...
    if (req.query.page === undefined && req.query.limit === undefined) {
      ...
      const totalSubCategoriesQuery = `SELECT COUNT(*) AS total FROM public.GEBC_sub_category`;
...
export const updateCatgory = async (req, res) => {
  try {
    ...
    const subCategoryQuery =
      "SELECT * FROM public.GEBC_sub_category WHERE id=$1";
    ...
    const updateSubCategory = `UPDATE public.GEBC_sub_category SET name=$1, category_id=$2, updated_at=NOW() WHERE id=$3 RETURNING *`;
...
export const deleteAllCategories = async (req, res) => {
  try {
    ...
    const query = "DELETE FROM public.GEBC_sub_category RETURNING *";
