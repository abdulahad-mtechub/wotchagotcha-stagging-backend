...
export const create = async (req, res) => {
  try {
    ...
    const createQuery =
      "INSERT INTO tv_progmax_videos (name, description, video, user_id, thumbnail, category_id, sub_category_id, shared_post_id) VALUES($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *";
    ...
    FROM tv_progmax_videos v
    JOIN users u ON v.user_id = u.id
    LEFT JOIN tv_progmax_category vc ON v.category_id = vc.id
    LEFT JOIN tv_progmax_sub_category vsc ON v.sub_category_id = vsc.id
    LEFT JOIN tv_progmax_videos orig ON v.shared_post_id = orig.id
    ...
    console.error(error);
    if (error.constraint === "tv_progmax_videos_user_id_fkey") {
      return res
        .status(400)
        .json({ statusCode: 400, message: "user not found" });
    } else if (error.constraint === "tv_progmax_videos_category_id_fkey") {
      return res
        .status(400)
        .json({ statusCode: 400, message: "category not found" });
    } else if (error.constraint === "tv_progmax_videos_sub_category_id_fkey") {
      return res
        .status(400)
...
export const update = async (req, res) => {
  try {
    ...
    const checkVideoQuery = "SELECT * FROM tv_progmax_videos WHERE id = $1";
    ...
    const updateQuery =
      "UPDATE tv_progmax_videos SET name=$1, description=$2, category_id=$3, sub_category_id=$4, video=$5, thumbnail=$6, user_id=$7 WHERE id=$8 RETURNING *";
...
export const deleteVideo = async (req, res) => {
  try {
    ...
    const deleteQuery = "DELETE FROM tv_progmax_videos WHERE id=$1 RETURNING *";
...
export const deleteAllVideos = async (req, res) => {
  try {
    const deleteQuery = "DELETE FROM tv_progmax_videos  RETURNING *";
...
export const addComment = async (req, res) => {
  try {
    ...
    const insertQuery =
      'INSERT INTO tv_progmax_video_comment (video_id, user_id, comment) VALUES ($1, $2, $3) RETURNING id, video_id, user_id, comment, created_at AS "commentCreatedAt", updated_at';
...
export const getComments = async (req, res) => {
  try {
    ...
    FROM tv_progmax_video_comment c
    ...
    const countQuery = `SELECT COUNT(*) FROM tv_progmax_video_comment WHERE video_id = $1`;
...
export const getSubcategoriesWithVideosByCategory = async (req, res) => {
  try {
    ...
    const checkCategoryQuery =
      "SELECT * FROM tv_progmax_category WHERE id = $1";
...
SELECT
  ...
  v.category_id,
  vc.name AS category_name,
  vsc.french_name AS category_french_name,
  v.sub_category_id,
  vsc.name AS sub_category_name,
...
LIMIT $2 OFFSET $3;
    ...
    const countQuery = `SELECT COUNT(*) FROM tv_progmax_videos WHERE user_id = $1`;
...
export const toggleLikeVideo = async (req, res) => {
  try {
    ...
    const checkLikeQuery =
      "SELECT * FROM tv_progmax_video_like WHERE video_id = $1 AND user_id = $2";
    ...
    if (checkLikeResult.rowCount > 0) {
      ...
      const unlikeQuery =
        "DELETE FROM tv_progmax_video_like WHERE video_id = $1 AND user_id = $2 RETURNING *";
      ...
      const likeQuery = `
        INSERT INTO tv_progmax_video_like (video_id, user_id)
        VALUES ($1, $2)
        RETURNING *;
      `;
...
export const getTvProgmaxLikes = async (req, res) => {
  try {
    const { video_id } = req.params;
    ...
    const query = `
      ...
      FROM tv_progmax_video_like 
      WHERE video_id = $1
    `;

    const detailsQuery = `
      SELECT * FROM tv_progmax_video_like 
      WHERE video_id = $1
    `;
...
export const searchVideosByTitle = async (req, res) => {
  try {
    ...
    const countQuery = `SELECT COUNT(*) FROM tv_progmax_videos WHERE name ILIKE $1`;
