...
export const create = async (req, res) => {
  try {
    ...
    const query = "SELECT * FROM cinematics_category WHERE id=$1";
    const category = await pool.query(query, [category_id]);
    if (category.rows.length === 0) {
      return res.status(404).json({ message: "Cinematic Catgory not found" });
    }
    const createQuery =
      "INSERT INTO cinematics_sub_category (category_id, name) VALUES ($1, $2) RETURNING *";
    const result = await pool.query(createQuery, [category_id, name]);

    if (result.rowCount === 1) {
      return res.status(201).json({
        statusCode: 201,
        message: "Cinematic category created successfully",
        data: result.rows[0],
      });
    }
...
export const deleteCategory = async (req, res) => {
  ...
  try {
    ...
    const oldCatgory = await getSingleRow("cinematics_sub_category", condition);
    if (oldCatgory.length === 0) {
      return res
        .status(404)
        .json({ statusCode: 404, message: "Cinematic category not found " });
    }
    const delQuery = "DELETE FROM cinematics_sub_category WHERE id=$1";
    await pool.query(delQuery, [id]);
    res.status(200).json({
      statusCode: 200,
      message: "Cinematic category deleted successfully",
...
export const getSpecificCategory = async (req, res) => {
  try {
    const { id } = req.params;
    const query = `SELECT sbc.*, c.name AS category_name, c.french_name AS category_french_name  FROM cinematics_sub_category sbc LEFT JOIN cinematics_category c ON sbc.category_id = c.id WHERE sbc.id = $1`;
...
export const getAllSubCategories = async (req, res) => {
  try {
    ...
    let supplierQuery = `SELECT sbc.*, c.name AS category_name , c.french_name AS category_french_name FROM cinematics_sub_category sbc LEFT JOIN cinematics_category c ON sbc.category_id = c.id  ORDER BY sbc.created_at DESC`;

    if (req.query.page === undefined && req.query.limit === undefined) {
      console.log("00000");
      // If no query parameters for pagination are provided, fetch all categories without pagination
      supplierQuery = `SELECT sbc.*, c.name AS category_name, c.french_name AS category_french_name  FROM cinematics_sub_category sbc LEFT JOIN cinematics_category c ON sbc.category_id = c.id  ORDER BY sbc.created_at DESC`;
    } else {
    ...
    if (req.query.page === undefined && req.query.limit === undefined) {
      ...
      const totalCategoriesQuery = `SELECT COUNT(*) AS total FROM public.cinematics_sub_category`;
...
export const getAllSubCategoriesByCategory = async (req, res) => {
  try {
    ...
    let supplierQuery = `
      ...
      FROM cinematics_sub_category sbc
      LEFT JOIN cinematics_category c ON sbc.category_id = c.id
    `;
    ...
    if (req.query.page === undefined && req.query.limit === undefined) {
      ...
      let totalCategoriesQuery = `SELECT COUNT(*) AS total FROM cinematics_sub_category`;
...
export const updateCatgory = async (req, res) => {
  try {
    ...
    const queryCategory = "SELECT * FROM cinematics_category WHERE id=$1";
    const querySubCategory =
      "SELECT * FROM cinematics_sub_category WHERE id=$1";
    ...
    if (oldCatgory.rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Cinematic Sub Catgory not found" });
    }

    const updateCategory = `UPDATE cinematics_sub_category SET name=$1, category_id =$2, updated_at=NOW() WHERE id=$3 RETURNING *`;
...
export const deleteAllCategory = async (req, res) => {
  try {
    ...
    const query = "DELETE FROM cinematics_sub_category RETURNING *";
    const { rows } = await pool.query(query);

    if (rows.length === 0) {
      return res.status(404).json({
        statusCode: 404,
        message: "No Cinematic category found to delete",
      });
    }

    res.status(200).json({
      statusCode: 200,
      message: "All Cinematic category deleted successfully",
