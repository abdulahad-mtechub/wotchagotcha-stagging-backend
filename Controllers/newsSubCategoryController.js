...
export const create = async (req, res) => {
  try {
    ...
    const createQuery =
      "INSERT INTO public.NEWS_sub_category (category_id, name) VALUES ($1, $2) RETURNING *";
...
export const deleteCategory = async (req, res) => {
  ...
  try {
    ...
    const delQuery = "DELETE FROM public.NEWS_sub_category WHERE id=$1";
...
export const getSpecificCategory = async (req, res) => {
  try {
    ...
    const result = await getSingleRow("public.NEWS_sub_category", condition);
    if (result.length === 0) {
      return res
        .status(404)
        .json({ statusCode: 404, message: "Sub-category not found" });
...
export const getAllSubCategories = async (req, res) => {
  try {
    ...
    let subCategoryQuery = `
      SELECT sc.*
      FROM public.NEWS_sub_category sc
    ...
    if (req.query.page === undefined && req.query.limit === undefined) {
      ...
      const totalSubCategoriesQuery = `SELECT COUNT(*) AS total FROM public.NEWS_sub_category`;
...
export const updateCategory = async (req, res) => {
  try {
    ...
    const subCategoryQuery =
      "SELECT * FROM public.NEWS_sub_category WHERE id=$1";
    ...
    const updateSubCategory = `UPDATE public.NEWS_sub_category SET name=$1, category_id=$2, updated_at=NOW() WHERE id=$3 RETURNING *`;
...
export const deleteAllCategory = async (req, res) => {
  try {
    ...
    const query = "DELETE FROM public.NEWS_sub_category RETURNING *";
