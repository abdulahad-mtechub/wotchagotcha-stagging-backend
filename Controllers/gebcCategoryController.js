...
export const create = async (req, res) => {
  try {
    const { name } = req.body;
    const createQuery =
      "INSERT INTO public.GEBC_category (name) VALUES ($1) RETURNING *";
...
export const deleteCategory = async (req, res) => {
  ...
  try {
    ...
    const oldCategory = await getSingleRow("public.GEBC_category", condition);
    if (oldCategory.length === 0) {
      return res
        .status(404)
        .json({ statusCode: 404, message: "Category not found" });
    }
    const delQuery = "DELETE FROM public.GEBC_category WHERE id=$1";
...
export const getSpecificCategory = async (req, res) => {
  try {
    ...
    const result = await getSingleRow("public.GEBC_category", condition);
    if (result.length === 0) {
      return res
        .status(404)
        .json({ statusCode: 404, message: "Category not found" });
...
export const getAllCategories = async (req, res) => {
  try {
    ...
    let categoryQuery = `
          ...
          FROM public.GEBC_category c
          LEFT JOIN public.GEBC_sub_category sc ON c.id = sc.category_id
          GROUP BY c.id
    ...
    if (req.query.page === undefined && req.query.limit === undefined) {
      ...
      const totalCategoriesQuery = `SELECT COUNT(*) AS total FROM public.GEBC_category`;
...
export const updateCatgory = async (req, res) => {
  try {
    ...
    const query = "SELECT * FROM public.GEBC_category WHERE id=$1";
    const oldCategory = await pool.query(query, [id]);
    if (oldCategory.rows.length === 0) {
      return res.status(404).json({ message: "Category not found" });
    }

    const updateCategory = `UPDATE public.GEBC_category SET name=$1, updated_at=NOW() WHERE id=$2 RETURNING *`;
...
export const deleteAllCategory = async (req, res) => {
  try {
    const query = "DELETE FROM public.GEBC_category RETURNING *";
