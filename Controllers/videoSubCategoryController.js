...
export const create = async (req, res) => {
  try {
    ...
    const createQuery =
      "INSERT INTO video_sub_category (category_id, name) VALUES ($1, $2) RETURNING *";
    const result = await pool.query(createQuery, [category_id, name]);

    if (result.rowCount === 1) {
      return res.status(201).json({
        statusCode: 201,
        message: "Video subcategory created successfully",
        data: result.rows[0],
      });
    }
...
export const deleteCategory = async (req, res) => {
  ...
  try {
    ...
    const oldCategory = await getSingleRow("video_sub_category", condition);
    if (oldCategory.length === 0) {
      return res
        .status(404)
        .json({ statusCode: 404, message: "Video subcategory not found" });
    }
    const delQuery = "DELETE FROM video_sub_category WHERE id=$1";
    await pool.query(delQuery, [id]);
    res.status(200).json({
      statusCode: 200,
      message: "Video subcategory deleted successfully",
...
export const getSpecificCategory = async (req, res) => {
  try {
    const { id } = req.params;
    const query = `SELECT vsc.*, vc.name AS category_name, vc.french_name AS category_french_name 
           FROM video_sub_category vsc 
               LEFT JOIN video_category vc ON vsc.category_id = vc.id 
               WHERE vsc.id = $1`;
...
export const getAllSubCategories = async (req, res) => {
  try {
    ...
    let subCategoryQuery = `SELECT vsc.*, vc.name AS category_name,
      vc.french_name AS category_french_name FROM video_sub_category vsc                         LEFT JOIN video_category vc ON vsc.category_id = vc.id  
      ORDER BY vsc.created_at DESC`;

    if (req.query.page === undefined && req.query.limit === undefined) {
      // If no query parameters for pagination are provided, fetch all subcategories without pagination
      subCategoryQuery = `SELECT vsc.*, vc.name AS category_name, vc.french_name AS category_french_name  FROM video_sub_category vsc LEFT JOIN video_category vc ON vsc.category_id = vc.id  ORDER BY vsc.created_at DESC`;
    } else {
    ...
    if (req.query.page === undefined && req.query.limit === undefined) {
      ...
      const totalCategoriesQuery = `SELECT COUNT(*) AS total FROM public.video_sub_category`;
...
export const getAllSubCategoriesByCategory = async (req, res) => {
  try {
    ...
    if (!isNaN(category_id)) {
      subCategoryQuery += ` WHERE vsc.category_id = $1`;
      queryParameters.push(category_id);
    }

    subCategoryQuery += ` ORDER BY vsc.created_at DESC`;

    if (req.query.page !== undefined && req.query.limit !== undefined) {
      subCategoryQuery += ` LIMIT $${queryParameters.length + 1} OFFSET $${
    ...
    if (req.query.page === undefined && req.query.limit === undefined) {
      ...
      // Calculate the total number of subcategories (without pagination)
      let totalCategoriesQuery = `SELECT COUNT(*) AS total FROM video_sub_category`;
...
export const updateCatgory = async (req, res) => {
  try {
    ...
    const queryCategory = "SELECT * FROM video_category WHERE id=$1";
    const querySubCategory = "SELECT * FROM video_sub_category WHERE id=$1";
    ...
    if (oldCategory.rows.length === 0) {
      return res.status(404).json({ message: "Video subcategory not found" });
    }

    const updateCategory = `UPDATE video_sub_category SET name=$1, category_id =$2, updated_at=NOW() WHERE id=$3 RETURNING *`;
    const result = await pool.query(updateCategory, [name, category_id, id]);
    if (result.rowCount === 1) {
      return res.status(200).json({
        statusCode: 200,
        message: "Video subcategory updated successfully",
...
export const deleteAllCategory = async (req, res) => {
  try {
    // Perform a query to delete all subcategories from the database
    const query = "DELETE FROM video_sub_category RETURNING *";
    const { rows } = await pool.query(query);

    if (rows.length === 0) {
      return res.status(404).json({
        statusCode: 404,
        message: "No video subcategory found to delete",
      });
    }

    res.status(200).json({
      statusCode: 200,
      message: "All video subcategories deleted successfully",
...
export const searchCategories = async (req, res) => {
  try {
    ...
    const userQuery = `SELECT * FROM video_sub_category WHERE ${conditions.join(
      " OR "
