...
export const create = async (req, res) => {
  try {
    ...
    LEFT JOIN kid_vids_category vc ON v.category_id = vc.id
    LEFT JOIN kid_vids_sub_category vsc ON v.sub_category_id = vsc.id
    ...
    console.error(error);
    if (error.constraint === "kid_vids_videos_user_id_fkey") {
      return res
        .status(400)
        .json({ statusCode: 400, message: "user not found" });
    } else if (error.constraint === "kid_vids_videos_category_id_fkey") {
      return res
        .status(400)
        .json({ statusCode: 400, message: "category not found" });
    } else if (error.constraint === "kid_vids_videos_sub_category_id_fkey") {
      return res
        .status(400)
...
export const update = async (req, res) => {
  try {
    ...
    const checkVideoQuery = "SELECT * FROM kid_vids_videos WHERE id = $1";
    ...
    const updateQuery =
      "UPDATE kid_vids_videos SET name=$1, description=$2, category_id=$3, sub_category_id=$4, video=$5, thumbnail=$6, user_id=$7 WHERE id=$8 RETURNING *";
...
export const deleteVideo = async (req, res) => {
  try {
    ...
    const deleteQuery = "DELETE FROM kid_vids_videos WHERE id=$1 RETURNING *";
...
export const deleteAllVideos = async (req, res) => {
  try {
    const deleteQuery = "DELETE FROM kid_vids_videos  RETURNING *";
...
export const addComment = async (req, res) => {
  try {
    ...
    const insertQuery =
      'INSERT INTO kid_vids_video_comment (video_id, user_id, comment) VALUES ($1, $2, $3) RETURNING id, video_id, user_id, comment, created_at AS "commentCreatedAt", updated_at';
...
export const getComments = async (req, res) => {
  try {
    ...
    const countQuery = `SELECT COUNT(*) FROM kid_vids_video_comment WHERE video_id = $1`;
...
ORDER BY v.created_at DESC
      ...
      if (subcategory_id && parseInt(subcategory_id) === subcategory.id) {
        ...
        countQuery = `SELECT COUNT(*) FROM kid_vids_videos WHERE sub_category_id = $1 AND status != 'blocked'`;
...
LIMIT $2 OFFSET $3;
    ...
    const countQuery = `SELECT COUNT(*) FROM kid_vids_videos WHERE user_id = $1`;
...
export const toggleLikeVideo = async (req, res) => {
  try {
    ...
    const checkLikeQuery =
      "SELECT * FROM kid_vids_video_like WHERE video_id = $1 AND user_id = $2";
    ...
    if (checkLikeResult.rowCount > 0) {
      ...
      const unlikeQuery =
        "DELETE FROM kid_vids_video_like WHERE video_id = $1 AND user_id = $2 RETURNING *";
      ...
      const likeQuery = `
        INSERT INTO kid_vids_video_like (video_id, user_id)
        VALUES ($1, $2)
        RETURNING *;
      `;
...
export const getKidVidsLikes = async (req, res) => {
  try {
    const { video_id } = req.params;
    ...
    const query = `
      ...
      FROM kid_vids_video_like 
      WHERE video_id = $1
    `;

    const detailsQuery = `
      SELECT * FROM kid_vids_video_like 
      WHERE video_id = $1
    `;
...
export const searchVideosByTitle = async (req, res) => {
  try {
    ...
    const countQuery = `SELECT COUNT(*) FROM kid_vids_videos WHERE name ILIKE $1`;
