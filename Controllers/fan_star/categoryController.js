...
export const create = async (req, res) => {
  try {
    const { name } = req.body;
    const createQuery =
      "INSERT INTO fan_star_category (name) VALUES ($1) RETURNING *";
    const result = await pool.query(createQuery, [name]);

    if (result.rowCount === 1) {
      return res.status(201).json({
        statusCode: 201,
        message: "Fan Star category created successfully",
        data: result.rows[0],
      });
    }
...
export const deleteCategory = async (req, res) => {
  ...
  try {
    ...
    const oldCatgory = await getSingleRow("fan_star_category", condition);
    if (oldCatgory.length === 0) {
      return res
        .status(404)
        .json({ statusCode: 404, message: "Fan Star category not found " });
    }
    const delQuery = "DELETE FROM fan_star_category WHERE id=$1";
    await pool.query(delQuery, [id]);
    res.status(200).json({
      statusCode: 200,
      message: "Fan Star category deleted successfully",
...
export const getSpecificCategory = async (req, res) => {
  try {
    ...
    const result = await getSingleRow("fan_star_category", condition);
    if (result.length === 0) {
      return res
        .status(404)
        .json({ statusCode: 404, message: "Fan Star category not found" });
...
export const getAllCategories = async (req, res) => {
  try {
    ...
    let supplierQuery = `
      ...
      FROM fan_star_category c
      LEFT JOIN fan_star_sub_category sc ON c.id = sc.category_id
      GROUP BY c.id
    ...
    if (req.query.page === undefined && req.query.limit === undefined) {
      ...
      const totalCategoriesQuery = `SELECT COUNT(*) AS total FROM public.fan_star_category`;
...
export const updateCatgory = async (req, res) => {
  try {
    ...
    if (oldCatgory.rows.length === 0) {
      return res.status(404).json({ message: "Fan Star Catgory not found" });
    }

    const updateCategory = `UPDATE fan_star_category SET name=$1, "updated_at"=NOW() WHERE id=$2 RETURNING *`;
...
export const deleteAllCategory = async (req, res) => {
  try {
    ...
    const query = "DELETE FROM fan_star_category RETURNING *";
    const { rows } = await pool.query(query);

    if (rows.length === 0) {
      return res.status(404).json({
        statusCode: 404,
        message: "No Fan Star category found to delete",
      });
    }

    res.status(200).json({
      statusCode: 200,
      message: "All Fan Star category deleted successfully",
