...
export const create = async (req, res) => {
  try {
    ...
    const createQuery =
      "INSERT INTO learning_hobbies_videos (name, description, video, user_id, thumbnail, category_id, sub_category_id, shared_post_id) VALUES($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *";
...
export const update = async (req, res) => {
  try {
    ...
    const checkVideoQuery =
      "SELECT * FROM learning_hobbies_videos WHERE id = $1";
    ...
    const updateQuery =
      "UPDATE learning_hobbies_videos SET name=$1, description=$2, category_id=$3, sub_category_id=$4, video=$5, thumbnail=$6, user_id=$7 WHERE id=$8 RETURNING *";
...
export const deleteVideo = async (req, res) => {
  try {
    ...
    const deleteQuery =
      "DELETE FROM learning_hobbies_videos WHERE id=$1 RETURNING *";
...
export const deleteAllVideos = async (req, res) => {
  try {
    const deleteQuery = "DELETE FROM learning_hobbies_videos  RETURNING *";
...
export const addComment = async (req, res) => {
  try {
    ...
    const insertQuery =
      'INSERT INTO learning_hobbies_video_comment (video_id, user_id, comment) VALUES ($1, $2, $3) RETURNING id, video_id, user_id, comment, created_at AS "commentCreatedAt", updated_at';
...
export const getComments = async (req, res) => {
  try {
    ...
    const countQuery = `SELECT COUNT(*) FROM learning_hobbies_video_comment WHERE video_id = $1`;
...
export const getSubcategoriesWithVideosByCategory = async (req, res) => {
  try {
    ...
    const checkCategoryQuery =
      "SELECT * FROM learning_hobbies_category WHERE id = $1";
    ...
    const subcategoriesQuery = `
      SELECT id, name, created_at, "index" AS sub_category_index
      FROM learning_hobbies_sub_category 
      WHERE category_id = $1
...
ORDER BY v.created_at DESC
      ...
      if (subcategory_id && parseInt(subcategory_id) === subcategory.id) {
        ...
        countQuery = `SELECT COUNT(*) FROM learning_hobbies_videos WHERE sub_category_id = $1 AND status != 'blocked'`;
...
LIMIT $2 OFFSET $3;
    ...
    const countQuery = `SELECT COUNT(*) FROM learning_hobbies_videos WHERE user_id = $1`;
...
export const toggleLikeVideo = async (req, res) => {
  try {
    ...
    const checkLikeQuery =
      "SELECT * FROM learning_hobbies_video_like WHERE video_id = $1 AND user_id = $2";
    ...
    if (checkLikeResult.rowCount > 0) {
      ...
      const unlikeQuery =
        "DELETE FROM learning_hobbies_video_like WHERE video_id = $1 AND user_id = $2 RETURNING *";
      ...
      const likeQuery = `
        INSERT INTO learning_hobbies_video_like (video_id, user_id)
        VALUES ($1, $2)
        RETURNING *;
      `;
...
export const getLearningAndHobbiesLikes = async (req, res) => {
  try {
    const { video_id } = req.params;
    ...
    const detailsQuery = `
      SELECT * FROM learning_hobbies_video_like 
      WHERE video_id = $1
    `;
...
export const searchVideosByTitle = async (req, res) => {
  try {
    ...
    const countQuery = `SELECT COUNT(*) FROM learning_hobbies_videos WHERE name ILIKE $1`;
