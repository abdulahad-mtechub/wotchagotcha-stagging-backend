...
export const create = async (req, res) => {
  try {
    const { name } = req.body;
    const createQuery =
      "INSERT INTO learning_hobbies_category (name) VALUES ($1) RETURNING *";
...
export const deleteCategory = async (req, res) => {
  ...
  try {
    ...
    const oldCatgory = await getSingleRow("learning_hobbies_category", condition);
    if (oldCatgory.length === 0) {
      return res
        .status(404)
        .json({ statusCode: 404, message: "category not found " });
    }
    const delQuery = "DELETE FROM learning_hobbies_category WHERE id=$1";
...
export const getSpecificCategory = async (req, res) => {
  try {
    ...
    const result = await getSingleRow("learning_hobbies_category", condition);
    if (result.length === 0) {
      return res
        .status(404)
...
export const getAllCategories = async (req, res) => {
  try {
    ...
    let supplierQuery = `
      ...
      FROM learning_hobbies_category c
      LEFT JOIN learning_hobbies_sub_category sc ON c.id = sc.category_id
      GROUP BY c.id
    ...
    if (req.query.page === undefined && req.query.limit === undefined) {
      ...
      const totalCategoriesQuery = `SELECT COUNT(*) AS total FROM public.learning_hobbies_category`;
...
export const updateCatgory = async (req, res) => {
  try {
    ...
    const updateCategory = `UPDATE learning_hobbies_category SET name=$1, "updated_at"=NOW() WHERE id=$2 RETURNING *`;
...
export const deleteAllCategory = async (req, res) => {
  try {
    ...
    const query = "DELETE FROM learning_hobbies_category RETURNING *";
