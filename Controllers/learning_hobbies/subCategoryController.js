...
export const create = async (req, res) => {
  try {
    ...
    const query = "SELECT * FROM learning_hobbies_category WHERE id=$1";
    const category = await pool.query(query, [category_id]);
    if (category.rows.length === 0) {
      return res.status(404).json({ message: "Catgory not found" });
    }
    const createQuery =
      "INSERT INTO learning_hobbies_sub_category (category_id, name) VALUES ($1, $2) RETURNING *";
...
export const deleteCategory = async (req, res) => {
  ...
  try {
    ...
    const oldCatgory = await getSingleRow(
    ...
    const delQuery = "DELETE FROM learning_hobbies_sub_category WHERE id=$1";
...
export const getSpecificCategory = async (req, res) => {
  try {
    const { id } = req.params;
    const query = `SELECT sbc.*, c.name AS category_name, c.french_name AS category_french_name FROM learning_hobbies_sub_category sbc LEFT JOIN learning_hobbies_category c ON sbc.category_id = c.id WHERE sbc.id = $1`;
...
export const getAllSubCategories = async (req, res) => {
  try {
    ...
    let supplierQuery = `SELECT sbc.*, c.name AS category_name, c.french_name AS category_french_name FROM learning_hobbies_sub_category sbc LEFT JOIN learning_hobbies_category c ON sbc.category_id = c.id  ORDER BY sbc.created_at DESC`;

    if (req.query.page === undefined && req.query.limit === undefined) {
      console.log("00000");
      // If no query parameters for pagination are provided, fetch all categories without pagination
      supplierQuery = `SELECT sbc.*, c.name AS category_name, c.french_name AS category_french_name FROM learning_hobbies_sub_category sbc LEFT JOIN learning_hobbies_category c ON sbc.category_id = c.id  ORDER BY sbc.created_at DESC`;
    } else {
    ...
    if (req.query.page === undefined && req.query.limit === undefined) {
      ...
      const totalCategoriesQuery = `SELECT COUNT(*) AS total FROM public.learning_hobbies_sub_category`;
...
export const getAllSubCategoriesByCategory = async (req, res) => {
  try {
    ...
    let supplierQuery = `
      ...
      FROM learning_hobbies_sub_category sbc
      LEFT JOIN learning_hobbies_category c ON sbc.category_id = c.id
    `;
    ...
    supplierQuery += ` ORDER BY sbc.created_at DESC`;
    ...
    if (req.query.page === undefined && req.query.limit === undefined) {
      ...
      let totalCategoriesQuery = `SELECT COUNT(*) AS total FROM learning_hobbies_sub_category`;
...
export const updateCatgory = async (req, res) => {
  try {
    ...
    const queryCategory = "SELECT * FROM learning_hobbies_category WHERE id=$1";
    const querySubCategory =
      "SELECT * FROM learning_hobbies_sub_category WHERE id=$1";
    ...
    if (oldCatgory.rows.length === 0) {
      return res.status(404).json({ message: "Sub Catgory not found" });
    }

    const updateCategory = `UPDATE learning_hobbies_sub_category SET name=$1, category_id =$2, updated_at=NOW() WHERE id=$3 RETURNING *`;
...
export const deleteAllCategory = async (req, res) => {
  try {
    ...
    const query = "DELETE FROM learning_hobbies_sub_category RETURNING *";
